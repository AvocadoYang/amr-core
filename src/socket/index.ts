...
export const startOneTermAllowPath$ = fromEventPattern<{
  amrId: string;
  start: boolean;
}>((next) => socket.on("start-initial", next)).pipe(share());
...
export const yellowImgLog$ = fromEventPattern<{ imgPath: string }>((next) =>
  socket.on("yellow-img-log-path", next)
).pipe(share());

export const heartbeat$ = fromEventPattern<{ payload: string }>((next) =>
  socket.on("heartbeat", next)
).pipe(share());

export function sendServiceYellowResult(
  image_data: string,
  result: boolean,
  result_msg: string
) {
  socket.volatile.emit("send-yellow-img-log", {
    image_data,
    result,
    result_msg,
  });
...
export function sendCarErrorInfo(errorInfo: {
  warning_msg: string[];
  warning_id: string[];
}) {
